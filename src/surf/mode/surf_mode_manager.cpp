...
void Surf::mode::InitModeManager()
{
	...
	ModeServiceFactory _64tFactory = [](SurfPlayer *player) -> SurfModeService * { return new Surf64tModeService(player); };
	modeManager.RegisterMode(g_PLID, "64t", "64tick", _64tFactory);
...
void Surf::mode::LoadModePlugins()
{
	char buffer[1024];
	g_SMAPI->PathFormat(buffer, sizeof(buffer), "addons/cs2surf/modes/*%s", MODULE_EXT);
	...
	if (output)
	{
		...
		do
		{
			g_SMAPI->PathFormat(fullPath, sizeof(fullPath), "%s/addons/cs2surf/modes/%s", g_SMAPI->GetBaseDir(), output);
			bool already = false;
...
void Surf::mode::UpdateModeDatabaseID(CUtlString name, i32 id, CUtlString shortName)
{
...
void Surf::mode::InitModeService(SurfPlayer *player)
{
	delete player->modeService;
	player->modeService = new Surf64tModeService(player);
}

void Surf::mode::DisableReplicatedModeCvars()
{
	for (u32 i = 0; i < MODECVAR_COUNT; i++)
	{
...
bool SurfModeManager::RegisterMode(PluginId id, const char *shortModeName, const char *longModeName, ModeServiceFactory factory)
{
	...
	V_snprintf(shortModeCmd, 64, "surf_%s", shortModeName);
	V_snprintf(shortModeDescription, 64, "Command Description - surf_%s", shortModeName);
	bool shortCmdRegistered = scmd::RegisterCmd(V_strlower(shortModeCmd), Command_SurfModeShort, shortModeDescription, SCFL_MODESTYLE);
	...
	if (!info)
	{
		...
		SurfDatabaseService::InsertAndUpdateModeIDs(longModeName, shortModeName);
	}
...
void SurfModeManager::UnregisterMode(PluginId id)
{
	...
	FOR_EACH_VEC(modeInfos, i)
	{
		if (id == modeInfos[i].id)
		{
			...
			char shortModeCmd[64];
			V_snprintf(shortModeCmd, 64, "surf_%s", modeInfos[i].shortModeName.Get());
...
bool SurfModeManager::SwitchToMode(SurfPlayer *player, const char *modeName, bool silent, bool force, bool updatePreference)
{
...
void SurfModeManager::Cleanup()
{
	...
	for (u32 i = 0; i < MODECVAR_COUNT; i++)
	{
		...
		Surf::mode::modeCvarRefs[i]->GetDefaultAsString(defaultValue);
		Surf::mode::modeCvarRefs[i]->SetString(defaultValue);
	}
}

SCMD(surf_mode, SCFL_MODESTYLE)
{
...
{
	SurfPlayer *player = g_pSurfPlayerManager->ToPlayer(controller);
	// Strip surf_ prefix if exist.
	size_t len = 0;
	bool stripped = false;
	if (!stripped)
	{
		len = strlen("surf_");
		stripped = strncmp(args->Arg(0), "surf_", len) == 0;
	}
	if (!stripped)
	{
		len = 1;
...
{
	SurfModeManager::ModePluginInfo emptyInfo;
	if (!mode)
	{
		META_CONPRINTF("[Surf] Warning: Getting mode info from a nullptr!\n");
		return emptyInfo;
	}
	FOR_EACH_VEC(modeInfos, i)
	{
...
SurfModeManager::ModePluginInfo Surf::mode::GetModeInfo(Surf::API::Mode mode)
{
	switch (mode)
	{
...
SurfModeManager::ModePluginInfo Surf::mode::GetModeInfo(CUtlString modeName)
{
	SurfModeManager::ModePluginInfo emptyInfo;
	if (modeName.IsEmpty())
	{
		META_CONPRINTF("[Surf] Warning: Getting mode info from an empty string!\n");
		return emptyInfo;
	}
	FOR_EACH_VEC(modeInfos, i)
	{
...
SurfModeManager::ModePluginInfo Surf::mode::GetModeInfoFromDatabaseID(i32 id)
{
	FOR_EACH_VEC(modeInfos, i)
	{
...
void SurfDatabaseServiceEventListener_Modes::OnDatabaseSetup()
{
	FOR_EACH_VEC(modeInfos, i)
	{
		if (modeInfos[i].databaseID == -1)
		{
			SurfDatabaseService::InsertAndUpdateModeIDs(modeInfos[i].longModeName, modeInfos[i].shortModeName);
		}
	}
