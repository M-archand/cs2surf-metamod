...
void SurfDatabaseService::SetupClient()
{
	...
	GetDatabaseConnection()->ExecuteTransaction(
		...
		{
			SurfPlayer *pl = g_pSurfPlayerManager->ToPlayer(userID);
			...
			if (result)
			{
				...
				CALL_FORWARD(SurfDatabaseService::eventListeners, OnClientSetup, pl, pl->GetSteamId64(), isCheater);
			}
		},
		OnGenericTxnFailure);
}
