...
void SurfDatabaseService::RunMigrations()
{
	Transaction txn;
	...
	GetDatabaseConnection()->ExecuteTransaction(txn, SurfDatabaseService::CheckMigrations, OnGenericTxnFailure);
}

void SurfDatabaseService::CheckMigrations(std::vector<ISQLQuery *> queries)
{
	...
	if (current > max)
	{
		META_CONPRINTF("[Surf::DB] Fatal error: Number of current migrations are higher than the maximum!\n");
		return;
	}
	...
	auto onSuccess = []()
	{
		META_CONPRINT("[Surf::DB] Database migration successful.\n");
...
bool SurfDatabaseService::IsReady()
{
