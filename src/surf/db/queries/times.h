...
constexpr char sql_times_insert[] = R"(
    INSERT INTO Times (ID, SteamID64, MapCourseID, ModeID, StyleIDFlags, RunTime, Metadata) 
        VALUES ('%s', %llu, %d, %d, %llu, %.7f, '%s')
)";
...
constexpr char sqlite_times_alter_id_column_2[] = R"(
    INSERT INTO Times_New (ID, SteamID64, MapCourseID, ModeID, StyleIDFlags, RunTime, Metadata, Created)
    SELECT CAST(ID as TEXT), SteamID64, MapCourseID, ModeID, StyleIDFlags, RunTime, Metadata, Created FROM Times
)";
