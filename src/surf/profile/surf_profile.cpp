...
{
	if (g_pSurfUtils->GetServerGlobals()->tickcount % 64 != 0)
	{
		return;
	}
...
void SurfProfileService::OnCheckTransmit()
{
...
void SurfProfileService::RequestRating()
{
	if (!SurfGlobalService::IsAvailable() || !this->player->IsAuthenticated() || !this->player->IsConnected())
	{
		return;
	}
	Surf::API::Mode mode;
	if (!Surf::API::DecodeModeString(this->player->modeService->GetModeShortName(), mode))
	{
		return;
	}
	...
	auto callback = [steamID64, mode](HTTP::Response response)
	{
		...
		SurfPlayer *player = g_pSurfPlayerManager->SteamIdToPlayer(steamID64);
		if (player == nullptr)
		{
			return;
		}
...
bool SurfProfileService::CanDisplayRank()
{
	...
	Surf::API::Mode mode;
	return Surf::API::DecodeModeString(this->player->modeService->GetModeShortName(), mode);
}

void SurfProfileService::UpdateClantag()
{
...
void SurfProfileService::OnPhysicsSimulatePost()
{
...
void SurfProfileService::UpdateCompetitiveRank()
{
	if (!this->player->GetController() || !surf_profile_rating_badge_enabled.GetBool())
	{
		return;
	}
...
std::string SurfProfileService::GetPrefix(bool colors)
{
	if (this->clanTag[0] == '\0')
	{
		this->UpdateClantag();
