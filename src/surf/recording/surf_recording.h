...
public:
	...
	SurfModeManager::ModePluginInfo lastKnownMode;
	std::vector<SurfStyleManager::StylePluginInfo> lastKnownStyles;
...
public:
	...
	void PushToRecorders(const SubtickData &value, RecorderType target = RecorderType::Run)
	{
		ApplyToTarget([&](auto &r) { r.template PushData<V>(value); }, target);
	}

	template<typename T>
	void PushToRecorders(const T &value, RecorderType target = RecorderType::Run)
	{
