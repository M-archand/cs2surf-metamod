...
void SurfCheckpointService::Init()
{
	SurfOptionService::RegisterEventListener(&optionEventListener);
}

void SurfCheckpointService::Reset()
{
...
void SurfCheckpointService::OnPlayerPreferencesLoaded()
{
	...
	CUtlString currentMap = g_pSurfUtils->GetCurrentMapName(&hasMapName);
	if (!hasMapName)
	{
		return;
	}
...
void SurfCheckpointService::ResetCheckpoints(bool playSound, bool resetTeleports)
{
...
void SurfCheckpointService::SetCheckpoint()
{
...
void SurfCheckpointService::UndoTeleport()
{
	CCSPlayerPawn *pawn = this->player->GetPlayerPawn();
...
void SurfCheckpointService::DoTeleport(i32 index)
{
...
void SurfCheckpointService::DoTeleport(const Checkpoint cp)
{
	...
	this->tpCount++;
	this->teleportTime = g_pSurfUtils->GetServerGlobals()->curtime;
	this->PlayTeleportSound();
	this->lastTeleportedCheckpoint = cp;
}

void SurfCheckpointService::TpToCheckpoint()
{
	DoTeleport(this->currentCpIndex);
}

void SurfCheckpointService::TpToPrevCp()
{
	if (this->checkpoints.Count() <= 0)
	{
		this->player->languageService->PrintChat(true, false, "Can't Teleport (No Checkpoints)");
...
void SurfCheckpointService::TpToNextCp()
{
	if (this->checkpoints.Count() <= 0)
	{
		this->player->languageService->PrintChat(true, false, "Can't Teleport (No Checkpoints)");
...
void SurfCheckpointService::TpHoldPlayerStill()
{
	bool isAlive = this->player->IsAlive();
	bool justTeleported = g_pSurfUtils->GetServerGlobals()->curtime - this->teleportTime > 0.04;
...
void SurfCheckpointService::SetStartPosition()
{
	CCSPlayerPawn *pawn = this->player->GetPlayerPawn();
	if (!pawn)
	{
...
void SurfCheckpointService::ClearStartPosition()
{
	this->hasCustomStartPosition = false;
