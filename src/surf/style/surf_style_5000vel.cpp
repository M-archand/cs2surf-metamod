#include "surf_style_5000vel.h"

#include "utils/addresses.h"
#include "utils/interfaces.h"
#include "utils/gameconfig.h"

Surf5000VelStylePlugin g_Surf5000VelStylePlugin;

CGameConfig *g_pGameConfig = NULL;
SurfUtils *g_pSurfUtils = NULL;
SurfStyleManager *g_pStyleManager = NULL;
StyleServiceFactory g_StyleFactory = [](SurfPlayer *player) -> SurfStyleService * { return new Surf5000VelStyleService(player); };
PLUGIN_EXPOSE(Surf5000VelStylePlugin, g_Surf5000VelStylePlugin);

CConVarRef<float> sv_maxvelocity("sv_maxvelocity");
const char *incompatibleStyles[] = {"3500vel", "10000vel"};

bool Surf5000VelStylePlugin::Load(PluginId id, ISmmAPI *ismm, char *error, size_t maxlen, bool late)
{
	PLUGIN_SAVEVARS();
	// Load mode
	int success;
...
bool Surf5000VelStylePlugin::Unload(char *error, size_t maxlen)
{
	g_pStyleManager->UnregisterStyle(g_PLID);
	return true;
}

bool Surf5000VelStylePlugin::Pause(char *error, size_t maxlen)
{
	g_pStyleManager->UnregisterStyle(g_PLID);
	return true;
}

bool Surf5000VelStylePlugin::Unpause(char *error, size_t maxlen)
{
...
void Surf5000VelStyleService::Init()
{
	g_pSurfUtils->SendConVarValue(this->player->GetPlayerSlot(), sv_maxvelocity, "5000");
}

const CVValue_t *Surf5000VelStyleService::GetTweakedConvarValue(const char *name)
{
...
void Surf5000VelStyleService::Cleanup()
{
...
void Surf5000VelStyleService::OnProcessMovement()
{
