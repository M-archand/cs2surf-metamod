...
namespace Surf::replaysystem::playback
{
	...
	void OnPhysicsSimulate(SurfPlayer *player)
	{
		...
		moveServices->m_LegacyJump().m_flJumpPressedTime =
			g_pSurfUtils->GetServerGlobals()->curtime + tickData->pre.jumpPressedTime - tickData->gameTime;

		moveServices->m_flDuckSpeed = tickData->pre.duckSpeed;
		moveServices->m_flDuckAmount = tickData->pre.duckAmount;
		moveServices->m_flLastDuckTime = g_pSurfUtils->GetServerGlobals()->curtime + tickData->pre.lastDuckTime - tickData->gameTime;
	...
	void OnPhysicsSimulatePost(SurfPlayer *player)
	{
		...
		moveServices->m_flLastDuckTime = g_pSurfUtils->GetServerGlobals()->curtime + tickData->post.lastDuckTime - tickData->gameTime;
		...
		if (replay->replayPaused)
		{
			...
			// Don't advance tick or process events
		...
		// Process events after physics simulation
	...
	void OnPlayerRunCommandPre(SurfPlayer *player, PlayerCommand *command)
	{
		...
		if (surf_replay_playback_debug.Get())
		{
	...
	void CheckWeapon(SurfPlayer &player, PlayerCommand &cmd)
	{
		...
		if (desiredWeapon != activeWeapon)
		{
			...
			gear_slot_t desiredGearSlot = Surf::replaysystem::item::GetWeaponGearSlot(desiredWeapon.mainInfo.itemDef);
			for (i32 i = 0; i < weapons->Count(); i++)
			{
				CBasePlayerWeapon *invWeapon = weapons->Element(i).Get();
				gear_slot_t invGearSlot =
					Surf::replaysystem::item::GetWeaponGearSlot(invWeapon->m_AttributeManager().m_Item().m_iItemDefinitionIndex());
			...
			std::string weaponName = Surf::replaysystem::item::GetWeaponName(desiredWeapon.mainInfo.itemDef);
			CBasePlayerWeapon *newWeapon = player.GetPlayerPawn()->m_pItemServices()->GiveNamedItem(weaponName.c_str());
			if (newWeapon)
			{
				Surf::replaysystem::item::ApplyItemAttributesToWeapon(*newWeapon, desiredWeapon);
