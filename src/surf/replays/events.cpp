...
namespace Surf::replaysystem::events
{
	...
	void HandleTimerEvent(SurfPlayer &player, const RpEvent *event, data::ReplayPlayback *replay)
	{
		...
		switch (event->data.timer.type)
		{
			...
			{
				const SurfCourseDescriptor *course = Surf::course::GetCourseByCourseID(event->data.timer.index);
				...
				replay->startTime = g_pSurfUtils->GetServerGlobals()->curtime - event->data.timer.time;
				replay->endTime = 0.0f;
			...
			{
				const SurfCourseDescriptor *courseDesc = Surf::course::GetCourseByCourseID(event->data.timer.index);
				...
				{
					SurfStyleService *style = player.styleServices[i];
	...
	void HandleModeChangeEvent(SurfPlayer &player, const RpEvent *event)
	{
		...
		g_pSurfModeManager->SwitchToMode(&player, modeNamePtr, true, true, false);
	}
	...
	void ReprocessEventsUpToTick(data::ReplayPlayback *replay, u32 targetTick)
	{
		...
		else
		{
			...
			g_pSurfModeManager->SwitchToMode(player, "64tick", true, false, false);
			g_pSurfStyleManager->ClearStyles(player, true, false);
		...
		for (u32 i = startEventIndex; i < replay->numEvents; i++)
		{
			...
			switch (event->type)
			{
				...
				case RPEVENT_MODE_CHANGE:
				{
					g_pSurfModeManager->SwitchToMode(player, event->data.modeChange.name, true, false, false);
					break;
				}
				case RPEVENT_STYLE_CHANGE:
				{
