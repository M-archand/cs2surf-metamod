...
void Surf::replaysystem::item::InitItemAttributes()
{
	KeyValues *kv = new KeyValues("items_game");
	kv->LoadFromFile(g_pFullFileSystem, "scripts/items/items_game.txt", "GAME");
	if (!kv)
	{
		Warning("[Surf] Failed to load items_game.txt for item attributes!\n");
		delete kv;
		return;
	}
	...
	if (!subKey)
	{
		Warning("[Surf] Failed to find attributes key in items_game.txt!\n");
		delete kv;
		return;
	}
	...
	if (!subKey)
	{
		Warning("[Surf] Failed to find paint_kits key in items_game.txt!\n");
		delete kv;
		return;
	}
	for (KeyValues *sub = subKey->GetFirstSubKey(); sub != NULL; sub = sub->GetNextKey())
	...
	if (!subKey)
	{
		Warning("[Surf] Failed to find items key in items_game.txt!\n");
		delete kv;
		return;
	}
	for (KeyValues *sub = subKey->GetFirstSubKey(); sub != NULL; sub = sub->GetNextKey())
...
std::string Surf::replaysystem::item::GetItemAttributeName(u16 id)
{
	return itemAttributes[id];
}

std::string Surf::replaysystem::item::GetWeaponName(u16 id)
{
...
gear_slot_t Surf::replaysystem::item::GetWeaponGearSlot(u16 id)
{
	return weaponItemMapping[id].gearSlot;
}

bool Surf::replaysystem::item::DoesPaintKitUseLegacyModel(float paintKit)
{
	return paintKitUsesLegacyModel[static_cast<u32>(paintKit)];
}

void Surf::replaysystem::item::ApplyItemAttributesToWeapon(CBasePlayerWeapon &weapon, const EconInfo &info)
{
	...
	bool isKnife = false;
	if (SURF_STREQI(weapon.GetClassname(), "weapon_knife") || SURF_STREQI(weapon.GetClassname(), "weapon_knife_t"))
	{
	...
	for (int i = 0; i < info.mainInfo.numAttributes; i++)
	{
		...
		if (id == 6)
		{
			...
			bool usesLegacyModel = Surf::replaysystem::item::DoesPaintKitUseLegacyModel(value);
			if (!isKnife)
			{
		...
		g_pSurfUtils->SetOrAddAttributeValueByName(&item.m_AttributeList(), Surf::replaysystem::item::GetItemAttributeName(id).c_str(), value);
		g_pSurfUtils->SetOrAddAttributeValueByName(&attributeList, Surf::replaysystem::item::GetItemAttributeName(id).c_str(), value);
	}
}

void Surf::replaysystem::item::ApplyModelAttributesToPawn(CCSPlayerPawn *pawn, const EconInfo &info, const char *modelName)
{
	...
	{
		g_pSurfUtils->SetModel(pawn, modelName);
	}
	else
	{
		META_CONPRINTF("[Surf] Model '%s' does not exist, skipping model application.\n", modelName);
	}
