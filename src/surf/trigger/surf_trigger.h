...
class SurfTriggerService : public SurfBaseService
{
public:
	...
	struct TriggerTouchTracker
	{
		...
		const SurfTrigger *surfTrigger = NULL;
		...
		bool CanEndTouch()
		{
			...
			if (!startedTouch)
			{
				META_CONPRINTF("[Surf::Trigger] EndTouch check is called before the start touch interaction even finished!\n");
				return false;
			}
			return true;
		}
...
inline bool operator==(const SurfTriggerService::PushEvent &left, const SurfTriggerService::PushEvent &right)
{
	assert(left.source);
	assert(right.source);
